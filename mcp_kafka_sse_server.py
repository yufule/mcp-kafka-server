...
from mcp.server.sse_adapter import SSEAdapter
...
# 创建FastAPI应用
app = FastAPI(title="Kafka MCP Server")
...
# 创建SSE适配器
sse_adapter = SSEAdapter(mcp)

# 注册MCP端点
app.include_router(sse_adapter.router)

# 添加首页说明
@app.get("/")
async def root():
    return {
        "message": "Kafka MCP服务器已启动",
        "description": "这是一个基于MCP协议的Kafka工具服务器，提供查询Topic信息、读写数据和回放历史数据的功能",
        "endpoints": {
            "mcp": "/mcp",
            "docs": "/docs"
        }
